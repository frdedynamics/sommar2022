...
class quaternions_to_euler(object):
    def __init__ (self):
        self.odometry_sub = rospy.Subscriber("/odometry/filtered", Odometry, self.odometry_callback) #Subscriber til Odometry topic
        self.Odometry_euler_pub = rospy.Publisher ("/raspberry_bot/Odometry_euler_odom", Odometry, queue_size=1) #Publiserer odometry i euler vinkler
    ...
    def odometry_callback(self, data):
        ...
        orientering_quaternions = data.pose.pose.orientation
        ...
        Odometry_euler.pose.pose.position.x = data.pose.pose.position.x
        Odometry_euler.pose.pose.position.y = data.pose.pose.position.y
        Odometry_euler.pose.pose.position.z = data.pose.pose.position.z
        Odometry_euler.pose.pose.orientation.x = roll
        Odometry_euler.pose.pose.orientation.y = pitch
