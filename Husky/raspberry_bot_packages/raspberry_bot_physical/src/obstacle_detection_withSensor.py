#!/usr/bin/env python

import rospy
from sensor_msgs.msg import LaserScan
from std_msgs.msg import Bool
import numpy as np

class obstacle_detection(object):

    def __init__(self):
        self.LaserScan_sub = rospy.Subscriber("/realsense/LaserScan", LaserScan, self.LaserScan_callback) #Subscriber til Lidar laserscan
        self.ObjectDetection_pub = rospy.Publisher ("/raspberry_bot/ObjectDetection", Bool, queue_size=1) #Publisher for ObjectDetection
	    self.Crashdetection_sub = rospy.Subscriber("/raspberry_bot/CrashSensor", Bool, self.Crashdetected_callback) #Subscriber til seriell node som varsler dersom crash sensor blir utlost

        self.ObjectDetected = False
	    self.crashDetected = False

        #Node skal varsle om at det er detektert objekt dersom > beamslimit antall
        #laserstraler viser at et objekt er naermere enn stopDistance meter
        self.stopDistance = 0.7
        self.beamslimit = 5

    def Crashdetected_callback(self, data):
	self.crashDetected = data

        #Metode som kjores hver gang det publiseres i /realsense/LaserScan topic
    def LaserScan_callback(self, data):
        LaserScan_data = data.ranges

        #Kontrollerer hvor mange laserstraler som er under grenseverdien og varsler
        #om at objekt er detektert dersom flere laserstraaler enn beamslimit er
        #under stopDistance
        beamsDetected = 0
        for x in LaserScan_data:
            if x < self.stopDistance:
                beamsDetected += 1
        if beamsDetected >= self.beamslimit:
            self.ObjectDetected = True
        else:
            self.ObjectDetected = False

	if self.crashDetected:
		self.ObjectDetected = True

        #Publiserer ObjectDetection status
        self.ObjectDetection_pub.publish(self.ObjectDetected)






def main():
    rospy.init_node('obstacle_detection', anonymous=True)
    obstacle_detectionr_object = obstacle_detection()
    try:
        rospy.spin()
    except KeyboardInterrupt:
        print("Shutting down")

if __name__ == '__main__':
    main()
