...
class row_end_detection(object):
    ...
    def LaserScan_callback(self, data):
        ...
        #Kontrollerer om straler lengst mot hoyre eller menstre gar mot max
        x = 0
        rowBeamsInf = 0
        while(x < self.rowsToWatch):
            if (LaserScan_data[x] == LaserScan_inf) | (LaserScan_data[len(LaserScan_data)-1 - x] == LaserScan_inf):
                rowBeamsInf += 1
            x += 1
