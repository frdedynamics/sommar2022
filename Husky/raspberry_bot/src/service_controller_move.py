...
class service_controller_move(object):
 ...
 #       self.GPS_orientation_sub = rospy.Subscriber("/raspberry_bot/Odometry_euler_GPS", Odometry, self.GPS_orientation_callback) #Subscriber til Odometry topic
        ...
        #Leser odometry data fra Odometry_euler topic
    def Odometry_callback(self, data):
        self.Odometry_data = data.pose.pose
	self.GPS_orientation_data = data.pose.pose #Setter denne inn her siden vi ikke har GPS atm
...
#    def GPS_orientation_callback(self, data):
    ...
    def move_command_callback(self, request):
        ...
        if request.name == "rotasjon":
            rospy.loginfo("move service: Rotating robot to %.2f radians orientation.", request.x)
        ...
        #Metode som roterer roboten basert pa GPS orientasjon
    def rotasjon(self, request):
        #Beregner target basert pa navaerende odometry
        target = request.x

        #Hoyre rotasjon
        if request.x >= self.GPS_orientation_data.orientation.z:

            while(self.GPS_orientation_data.orientation.z < target):
